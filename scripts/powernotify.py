...
while True:
    ...
    elif status == True and last_status != 1:
        message(reason = 1)

    # Check if the device's battery is full Note this might be different on some laptops
    if percentage > float(full_battery_level) and last_percentage < float(full_battery_level):
        message(reason = 5)

    if status == False:

        # Check if the device's battery is below 15
        if percentage < float(low_battery_level) and last_percentage > float(low_battery_level):
            message(reason = 3)

        # Check if the device's battery is below 3
        if percentage < float(empty_battery_level) and last_percentage > float(empty_battery_level):
            message(reason = 4)            
